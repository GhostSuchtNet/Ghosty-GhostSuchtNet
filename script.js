...
function shouldUsePythonAutomatically(message, fileResult) {
    ...
    return /\b(?:berechne|rechne|durchschnitt|median|standardabweichung|statistik|matrix|gleichung|simulation|diagramm|balkendiagramm|liniendiagramm)\b|\d+\s*[*×÷/^+]\s*\d+/i.test(text);
}
...
async function sendText(messages, webResult, systemInstruction, request) {
    const response = await fetchWithClientTimeout("/api/ai/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
