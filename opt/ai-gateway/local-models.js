...
function requestBodyFor(config, body, image = null) {
    ...
    return {
        ...
        chat_template_kwargs: { enable_thinking: thinking !== "instant" },
...
async function chat(id, body, signal, image = null) {
    ...
    const thinking = ["instant", "low", "medium", "high"].includes(body?.thinking) ? body.thinking : "instant";
    const requestBody = requestBodyFor(config, body, image);
...
async function chatStream(id, body, signal, onToken) {
    const config = configFor(id);
    if (!config.url) {
        const error = new Error(`${config.label} ist noch nicht konfiguriert.`);
        error.status = 503;
    ...
    if (body?.image) {
        const error = new Error("Streaming wird für Bildanfragen nicht verwendet.");
        error.status = 400;
        throw error;
    }
    ...
    const thinking = ["instant", "low", "medium", "high"].includes(body?.thinking) ? body.thinking : "instant";
    return queues[id].run(async () => {
        const requestBody = { ...requestBodyFor(config, body), stream: true };
        ...
        if (!response.ok || !response.body) {
            ...
            const error = new Error(detail || `${config.label} HTTP ${response.status}`);
        ...
        const parseLine = line => {
            ...
            try {
                ...
                emit(choice?.delta?.content ?? choice?.text ?? "");
            } catch (error) {
                console.error(`[Local ${id}] Ungültiger Streaming-Chunk`, error.message);
            }
            return false;
        };
        let done = false;
        while (!done) {
            ...
            buffer = lines.pop() || "";
            for (const line of lines) if (parseLine(line)) done = true;
            if (chunk.done) break;
        }
        if (buffer.trim()) parseLine(buffer);
        if (!reply.trim()) throw Object.assign(new Error(`${config.label} hat keine Antwort geliefert.`), { status: 502 });
        return {
            reply: reply.trim(), provider: "Lokal", model: config.label, selectedModel: id,
            thinkingRequested: thinking, thinkingEffective: thinking, queue: queues[id].snapshot()
        };
    }, signal);
}
