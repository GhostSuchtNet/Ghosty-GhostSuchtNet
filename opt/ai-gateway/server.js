...
function toolContext(req, signal) {
    const allowedModels = allowedModelIdsFromAuth(req);
...
app.post(
    ...
    async (req, res) => {
        ...
        const allowed = allowedModelIdsFromAuth(req);
        if (!LOCAL_MODEL_IDS.has(requested) || !allowed.has(requested)) {
            return res.status(400).json({ error: "Streaming ist nur für erlaubte lokale Modelle verfügbar." });
        }
        ...
        res.flushHeaders();
        try {
            const result = await localModels.chatStream(requested, req.body, signal, token => {
            ...
            if (signal.aborted || res.writableEnded) return;
            console.error(`[Local stream ${requested}]`, error.message);
            res.write(`data: ${JSON.stringify({ type: "error", error: error.message || "Lokales Modell nicht erreichbar.", status: error.status || 502 })}\n\n`);
            res.end();
        }
    }
);
...
app.post(
    ...
    async (req, res) => {
        const signal =
            createRequestSignal(req, res);

        try {
            const context =
                toolContext(req, signal);

            const request = {
                text: requestText(req.body),
            ...
            if (
                ...
                const toolContextText =
                    [
                        "Ein spezialisiertes Tool hat bereits folgende verlässliche Daten ermittelt:",
                        "",
                        decision.result.text,
                        "",
                        JSON.stringify(
                        ...
                        "",
                        "Verwende diese Daten als Grundlage. Erfinde keine abweichenden Tool-Ergebnisse."
                    ].join("\n");
