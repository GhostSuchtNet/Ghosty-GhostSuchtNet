...
function createAuth() {
    ...
    async function status(req, res) {
        ...
        return res.json({
            ...
            blockedForSeconds: block.blocked ? Number(block.blockedForSeconds || 0) : 0
        });
    }
    ...
    async function state(req, res) {
        ...
        if (block.blocked) {
            return res.json({
                loggedIn: false,
                blocked: true,
                blockedForSeconds: Number(block.blockedForSeconds || 0)
            });
        }
        ...
        if (
            !row ||
            !row.enabled ||
            isExpired(row.session_expires_at) ||
            isExpired(row.code_expires_at)
        ) {
            if (row?.session_id) {
                await pool.execute(
                    "DELETE FROM access_sessions WHERE id = ?",
                    [row.session_id]
                );
            }
